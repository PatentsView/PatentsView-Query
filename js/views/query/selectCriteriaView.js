...
define([
  ...
  SelectCriteriaPatentTemplate,
  SelectCiteriaAssigneeTemplate,
  SelectCiteriaInventorTemplate,
  ExpressionsTemplate,
  AddCriteriaTemplate,
  AddCriteriaOperatorTemplate,
  AddCriteriaValueTemplate,
  OverlayModel,
  Overlay
) {
  var SelectCriteriaView = Backbone.View.extend({
    ...
    initialize: function (options) {
      ...
      this.selectCriteriaPatentTemplate = Handlebars.compile(
        SelectCriteriaPatentTemplate
      );
      this.selectCriteriaAssigneeTemplate = Handlebars.compile(
        SelectCiteriaAssigneeTemplate
      );
      this.selectCriteriaInventorTemplate = Handlebars.compile(
        SelectCiteriaInventorTemplate
      );
      this.expressionsTemplate = Handlebars.compile(ExpressionsTemplate);
      this.addTemplate = Handlebars.compile(AddCriteriaTemplate);
      this.addOperatorTemplate = Handlebars.compile(
        AddCriteriaOperatorTemplate
      );
    ...
    isValid: function () {
      if (this.model.get("isQuickSearch") == true) {
        ...
        $(this.el)
          .find("#filters")
          .html(
            '<span id="submitSearch-error" class="error">Please add search criteria for the query.</span>'
          );
    ...
    updateModel: function () {
      ...
      _.forEach(entities, function (opt) {
        opt.isActive = opt.id == entityId;
      });
    ...
    renderCategoryCriteria: function () {
      ...
      var cpcCondHtml = this.addTemplate({
        ...
        tip:
          "Metadata describing the cooperative patent classification of the patents. The full class string can be searched in CPC Class field with type-ahead functionality.",
      ...
      var nberCondHtml = this.addTemplate({
        ...
        name: "NBER",
        tip:
          "Metadata describing the technology area of the patents. See www.nber.org/patents for more information on NBER technology areas. Only available through May 2015.",
      ...
      var ipcCondHtml = this.addTemplate({
        ...
        tip:
          "Metadata describing the international patent classification of the patents. The full class string can be searched in IPC Class field with type-ahead functionality.",
      ...
      var uspcsCondHtml = this.addTemplate({
        ...
        tip:
          "Metadata describing the US patent classification of the patents. The full class string can be searched in USPC Class field with type-ahead functionality. Only available through May 2015.",
      ...
      var wiposCondHtml = this.addTemplate({
        ...
        name: "WIPO",
        tip:
          "WIPO tooltip should read: Metadata describing the technology field of the patents.",
      ...
      var citedPatentsCondHtml = this.addTemplate({
        ...
        name: "Cited Patents",
        tip:
          "Data related to patents that have been cited by the patents associated with the search criteria",
      ...
      var applicationCitationsCondHtml = this.addTemplate({
        ...
        tip:
          "Data related to patent applications that have been cited by the patents associated with the search criteria",
      ...
      var citedbyPatentsCondHtml = this.addTemplate({
        ...
        name: "Citing Patents",
        tip:
          "Data related to patents that have cited the patents associated with the search criteria",
      ...
      switch (entityId) {
        case "inventor": {
          $(this.el)
            .find("#conditions")
            .html(this.selectCriteriaInventorTemplate({}));
        ...
        default: //patent
        {
          $(this.el)
            .find("#conditions")
            .html(this.selectCriteriaPatentTemplate({}));
          $(this.el).find("#patent-condition").html(patentConditionHtml);
          $(this.el).find("#inventor-condition").html(inventorConditionHtml);
          $(this.el).find("#assignee-condition").html(assigneeConditionHtml);
          $(this.el).find("#lawyer-condition").html(lawyerConditionHtml);
          ...
          $(this.el)
            .find("#citedApplications-condition")
            .html(applicationCitationsCondHtml);
          $(this.el)
            .find("#citingPatents-condition")
            .html(citedbyPatentsCondHtml);
    ...
    keyLocation: function (e) {
      ...
      if (source.hasClass("state")) {
        ...
        if (_.isUndefined(attr) || attr == false) {
          $(source)
            .popover({
              content: "States data is only available for the United States.",
              placement: "top",
            })
            .popover("show");
        }
      ...
      if (source.hasClass("city")) {
        ...
        if (_.isUndefined(attr) || attr == false) {
          $(source)
            .popover({ content: "Specify a country first.", placement: "top" })
            .popover("show");
        }
    ...
    changeCondition: function (e) {
      ...
      if (isFieldChange) {
        ...
        //update the operator.
        $(this.el)
          .find("#add-" + source.data("id") + "-operator-container")
          .html(this.addOperatorTemplate(field));
        //update the value.
        $(this.el)
          .find("#add-" + source.data("id") + "-value-container")
          .html(this.addValueTemplate(field));

        if (field.type == "location") {
          var locationCountry = $(this.el).find(
            "#add-" +
              source.data("id") +
              '-value-container input[data-location-type="country"]'
          );
          var locationCity = $(this.el).find(
            "#add-" +
              source.data("id") +
              '-value-container input[data-location-type="city"]'
          );
          var locationState = $(this.el).find(
            "#add-" +
              source.data("id") +
              '-value-container input[data-location-type="state"]'
          );
        ...
        var valueField = $(this.el)
          .find("#add-" + field.optgroup + "-value-container")
          .find("#add-" + field.id + "-value");
    ...
    addCondition: function (e) {
      ...
      if (form.valid()) {
        ...
        var operator = $(this.el).find(
          "#add-" + fieldVal + "-operator option:selected"
        );
        ...
        if (filter.type == "location") {
          ...
          var ipc_main_group =
            val.length > 4 && splitClass.length > 0
              ? splitClass[0].substr(4)
              : "";
          var ipc_subgroup =
            val.length > 4 && splitClass.length > 1 ? splitClass[1] : "";
        ...
        if (!_.isUndefined(filtersDesc)) filtersDesc.remove();

        if (!_.isUndefined(error)) error.remove();

        this.model.set("expressions", expressions);
        $(this.el)
          .find("#filters")
          .html(this.expressionsTemplate(this.model.toJSON()));
        validator.resetForm();
        this.model.set("isQuickSearch", false);
        this.resetFields(id);
      }
    ...
    resetFields: function (id) {
      ...
      //update the operator.
      $(this.el)
        .find("#add-" + id + "-operator-container")
        .html(this.addOperatorTemplate({ id: id }));
      //update the value.
      $(this.el)
        .find("#add-" + id + "-value-container")
        .html(this.addValueTemplate({ id: id, reset: true }));
    },
    ...
    clearCondition: function (e) {
      ...
      if ($(this.el).find(".expression").length == 0) {
        //No expressions exist so reset the expressions template.
        this.model.set("expressions", []);
        $(this.el)
          .find("#filters")
          .html(this.expressionsTemplate(this.model.toJSON()));
    ...
    selectEntity: function (e) {
      ...
      if (
        ...
        !_.isEmpty(entityId) &&
        dataId != entityId &&
        (quickSearch != "" || expressions.length > 0)
      ) {
        $(".entity-change-modal")
          .find(".ok")
          .on("click", function () {
        ...
        $(".entity-change-modal")
          ...
          .on("click", function () {
            $("#categories")
              .find("input[type=radio][name=entity]")
              .prop("checked", false);
            $("#categories")
              .find("input[type=radio][value=" + entityId + "]")
              .prop("checked", true);
            $(".entity-change-modal").modal("hide");
          });
