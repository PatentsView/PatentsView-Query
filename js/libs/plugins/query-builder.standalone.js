...
(function (root, factory) {
    ...
    QueryBuilder.DEFAULTS = {
        ...
        operators: [
            { type: 'equal', nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'boolean', 'location'] },
            ...
            { type: 'any', nb_inputs: 1, multiple: false, apply_to: ['string']},
            { type: 'all', nb_inputs: 1, multiple: false, apply_to: ['string'] },
            { type: 'phrase', nb_inputs: 1, multiple: false, apply_to: ['string'] }
        ],
    ...
    QueryBuilder.prototype.displayError = function (node) {
        if (this.settings.display_errors) {
            if (node.error === null) {
                ...
                node.$el.find('.error-container .control-label').empty();
            }
            else {
                ...
                node.$el.find('.error-container .control-label').html(fmt.apply(null, error));
            }
        }
    };
    ...
    /**
      * Get an object representing current criteria
      * @return {object}
      */
    QueryBuilder.prototype.getCriteria = function () {
        ...
        var out = (function parse(group) {
            ...
            group.each(function (model) {
                ...
                data.criterion.push(rule);

            }, function (model) {
                data.criterion.push(parse(model));
            });
            ...
            switch (filter.input) {
                ...
                default:
                    switch (QueryBuilder.types[filter.type]) {
                        ...
                        case 'location':
                            if (value[i] === undefined || value[i].length === 0) {
                                result = ['location_empty'];
                                break;
                            }
                            if (value[i][0] === undefined || value[i][0].length === 0) {
                                //country is at index 0;
                                result = ['location_empty'];
                                break;
                            }
        ...
        if (filter.valueGetter) {
            ...
            for (var i = 0; i < operator.nb_inputs; i++) {
                ...
                switch (filter.input) {
                    ...
                    case 'location':
                        tmp =[];
                        tmp.push($value.find('[name=' + name + '_country]').val());
                        tmp.push($value.find('[name=' +name + '_city]').val());
                        tmp.push($value.find('[name=' +name + '_state]').val());
                        value.push(tmp);
                        break;
        ...
        if (filter.valueSetter) {
            ...
            for (var i = 0; i < operator.nb_inputs; i++) {
                ...
                case 'checkbox':
                    ...
                    case 'location':
                    $value.find('[name=' + name + '_country]').val(value[i][0]).trigger('change');
                    $value.find('[name=' + name + '_city]').val(value[i][1]).trigger('change');
                    $value.find('[name=' + name + '_state]').val(value[i][2]).trigger('change');
                    break;
...
<dl id="'+ group_id + '" class="rules-group-container"> \
  <dt class="rules-group-header"> \
    <div class="pull-right group-actions"> \
      <button type="button" class="btn btn-default" data-add="rule"> \
        <i class="' + this.icons.add_rule + '"></i> ' + this.lang.add_rule + ' \
      </button> \
      '+ (this.settings.allow_groups === -1 || this.settings.allow_groups >= level ?
                '<button type="button" class="btn btn-default" data-add="group"> \
          <i class="' + this.icons.add_group + '"></i> ' + this.lang.add_group + ' \
        </button>'
              : '') + ' \
      '+ (level > 1 ?
                '<button type="button" class="btn btn-warning" data-delete="group"> \
        ...
        QueryBuilder.prototype.getRuleInput = function (rule, value_id) {
            ...
            if (typeof filter.input === 'function') {
                ...
                switch (filter.input) {
                    ...
                    case 'location':
                        h += '<input class="form-control" type="text" data-location-type="country" data-provide="typeahead" autocomplete="false" name="' +name + '_country"';
                        h += ' placeholder="Country"';
                        h += '>';
                        h += '<input class="form-control" type="text" data-location-type="city" data-provide="typeahead" autocomplete="false" name="' +name + '_city"';
                        h += ' placeholder="City"';
                        h += '>';
                        h += '<input class="form-control" type="text" data-location-type="state" data-provide="typeahead" autocomplete="false" name="' +name + '_state"';
                        h += ' placeholder="State"';
                        h += '>';
                        break;
...
<div'+ style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  <label for="'+ id + '">' + val + '</label> \
        ...
        QueryBuilder.define('location', function (options) {
             //init country typeahead.
            this.on('afterCreateRuleInput', function (e, rule) {

                var locationCountry = rule.$el.find('.rule-value-container input[data-location-type="country"]');
                var locationCity = rule.$el.find('.rule-value-container input[data-location-type="city"]');
                var locationState = rule.$el.find('.rule-value-container input[data-location-type="state"]');

                var countryEngine = new Bloodhound({
                    datumTokenizer : Bloodhound.tokenizers.whitespace,
                    queryTokenizer: Bloodhound.tokenizers.whitespace,
                    local: ["Afghanistan", "Aland Islands", "Albania", "Algeria", "American Samoa", "Andorra", "Angola", "Anguilla", "Antigua and Barbuda", "Argentina", "Armenia", "Aruba", "Australia", "Austria", "Azerbaijan", "Bahamas", "Bahrain", "Bangladesh", "Barbados", "Belarus", "Belgium", "Belize", "Benin", "Bermuda", "Bolivia", "Bosnia and Herzegovina", "Botswana", "Brazil", "Brunei Darussalam", "Bulgaria", "Burkina Faso", "CA", "Cambodia", "Cameroon", "Canada", "Cayman Islands", "CH", "Chad", "Chile", "China", "CN", "Colombia", "Congo", "Congo, The Democratic Republic of the", "Cook Islands", "Costa Rica", "Cote d'Ivoire", "Croatia", "Cuba", "Curacao", "Cyprus", "Czech Republic", "Denmark", "Dominica", "Dominican Republic", "Ecuador", "Egypt", "El Salvador", "Eritrea", "ES", "Estonia", "Ethiopia", "Faroe Islands", "Fiji", "Finland", "FR", "France", "French Guiana", "French Polynesia", "Gabon", "Gambia", "Georgia", "Germany", "Ghana", "Gibraltar", "Greece", "Greenland", "Guadeloupe", "Guam", "Guatemala", "Guernsey", "Guyana", "Haiti", "Honduras", "Hong Kong", "HU", "Hungary", "Iceland", "IL", "India", "Indonesia", "Iran, Islamic Republic of", "Iraq", "Ireland", "Isle of Man", "Israel", "Italy", "Jamaica", "Japan", "Jersey", "Jordan", "JP", "Kazakhstan", "Kenya", "Korea, Democratic People's Republic of", "Korea, Republic of", "Kuwait", "Kyrgyzstan", "Lao People's Democratic Republic", "Latvia", "Lebanon", "Liberia", "Libyan Arab Jamahiriya", "Liechtenstein", "Lithuania", "Luxembourg", "Macao", "Macedonia", "Madagascar", "Malawi", "Malaysia", "Maldives", "Mali", "Malta", "Marshall Islands", "Martinique", "Mauritius", "Mayotte", "Mexico", "Moldova, Republic of", "Monaco", "Mongolia", "Montenegro", "Morocco", "Myanmar", "Namibia", "Nepal", "Netherlands", "New Caledonia", "New Zealand", "Nicaragua", "Niger", "Nigeria", "Northern Mariana Islands", "Norway", "Oman", "Pakistan", "Palau", "Palestinian Territory", "Panama", "Papua New Guinea", "Paraguay", "Peru", "Philippines", "Pitcairn", "Poland", "Portugal", "Puerto Rico", "Qatar", "Reunion", "Romania", "Russian Federation", "Saint Kitts and Nevis", "Saint Lucia", "Saint Vincent and the Grenadines", "Samoa", "San Marino", "Saudi Arabia", "Senegal", "Serbia", "Seychelles", "Singapore", "Sint Maarten", "SK", "Slovakia", "Slovenia", "Solomon Islands", "South Africa", "Spain", "Sri Lanka", "Sudan", "Suriname", "Swaziland", "Sweden", "Switzerland", "Syrian Arab Republic", "Taiwan", "Tajikistan", "Tanzania, United Republic of", "Thailand", "Trinidad and Tobago", "Tunisia", "Turkey", "Turkmenistan", "Uganda", "Ukraine", "United Arab Emirates", "United Kingdom", "United States", "Uruguay", "Uzbekistan", "Vanuatu", "Venezuela", "Vietnam", "Virgin Islands, British", "Virgin Islands, U.S.", "Yemen", "Zambia", "Zimbabwe"],
                    remote: {
                        url: "/querytool/query/location.php",
                ...
                var stateEngine = new Bloodhound({
                    ...
                    remote: {
                        url: "/querytool/query/location.php",
                                replace: function (url, query) {
                                    return url + "?cc=" +locationCountry.typeahead('val') + "&r=s" + "&s=" +query;
                                }
                            },
                    initialize: true
                });
         ...
         * Allows to export rules as a API statement.
         */
        ...
        QueryBuilder.defaults({
            apiOperators: {
                equal: function (r, v) {
                    ...
                    if (r.input === 'location') {
                        for (var i = 0; i < v.length; i++) {
                            var field = r.field;
                            if (i == 0) {
                                //Quick fix. Swap the country name for the code.
                                var country = $.grep(this.settings.countries, function (e) { return e.name === v[i]; });
                                part[field.replace(/location/g, 'country')] = (country.length == 0) ? v[i] : country[0].id;
                            } else if (i == 1) {
                                part[field.replace(/location/g, 'city')] = v[i];
                            } else {
                                part[field.replace(/location/g, 'state')] = v[i];
                            }
                        }
                    } else {
                        part[r.field] = v[0];
                ...
                less_or_equal: function (r, v) {
                    var part = {};
                    part[r.field] = v[0];

                    return { '_lte': part };
                },
                greater: function (r, v) {
                    var part = {};
                    part[r.field] = v[0];

                    return { '_gt': part };
                },
                greater_or_equal: function (r, v) {
                    var part = {};
                    part[r.field] = v[0];

                    return { '_gte': part };
                },
                between: function (r, v) {
                    var part = {};
                    part[r.field] = v[0];

                    return { '_between': part };
                },//Not Supported.
                begins_with: function (r, v) {
                    var part = {};
                    part[r.field] = '^' + escapeRegExp(v[0]);

                    return { '_begins': part };
                },
                not_begins_with: function (r, v) {
                    var part = {};
                    part[r.field] = '^' + escapeRegExp(v[0]);

                    return { '_nbegins': part };
                },//Not Supported.
                contains: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]);

                    return { '_contains': part };
                },
                not_contains: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]);

                    return { '_ncontains': part };
                }, //Not Supported.
                ends_with: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]) + '$';

                    return { '_ew': part };
                }, //Not Supported.
                not_ends_with: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]) + '$';

                    return { '_new': part};
                },//Not Supported.
                is_empty: function (r, v) {
                    var part = {};
                    part[r.field] = '';

                    return part;
                },//Not Supported.
                is_not_empty: function (r, v) {
                    var part = {};
                    part[r.field] = '';

                    return { 'neq': part };
                },//Not Supported.
                is_null: function (r, v) {
                    var part = {};
                    part[r.field] = null;

                    return part;
                },//Not Supported.
                is_not_null: function (r, v) {
                    var part = {};
                    part[r.field] = null;
                ...
                all: function (r, v) {
                    ...
                    return { '_text_all': part };
                },
                any: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]);

                    return { '_text_any': part };
                },
                phrase: function (r, v) {
                    var part = {};
                    part[r.field] = escapeRegExp(v[0]);

                    return { '_text_phrase': part };
                }
            }
        });
        ...
        QueryBuilder.extend({
            ...
            getQuery: function (data) {
                ...
                var that = this;
                var isSimplePair = true;

                var query = (function parse(data) {
                    if (!data.condition) {
                        data.condition = that.settings.default_condition;
                    }
                    if (['AND', 'OR'].indexOf(data.condition.toUpperCase()) === -1) {
                        error('Unable to build API query with condition "{0}"', data.condition);
                    }
                    ...
                    data.rules.forEach(function(rule) {
                        ...
                        if (rule.rules && rule.rules.length > 0) {
                            ...
                            //If we find anything other than a simple equal then this is not a simple pair.
                            if (isSimplePair && rule.operator !== "equal") {
                                isSimplePair = false;
                            }

                            var mdb = that.settings.apiOperators[rule.operator],
                                ope = that.getOperatorByType(rule.operator),
                                values = [];

                            if (mdb === undefined) {
                                error('Unknown API operation for operator "{0}"', rule.operator);
                            }
                            ...
                            // part[rule.field] = mdb.call(that, rule.field, values);
                            parts.push(mdb.call(that, rule, values));
                        }
                    });
                    ...
                    if (parts.length > 0) {
                        ...
                        res['_' + data.condition.toLowerCase()] = parts;
                    }
                    return res;
                }(data));

                if (isSimplePair) {
                    var pairs = query[Object.keys(query)[0]];
                    var pairsObject = {};

                    if (pairs !== undefined) {
                        for (var i = 0; i < pairs.length; i++) {
                            var name = Object.keys(pairs[i])[0];
                            pairsObject[name] = pairs[i][name];
                        }
                    }
        ...
        QueryBuilder.defaults({
            ...
            countries: [{ "id": "AF", "name": "Afghanistan" }, { "id": "AX", "name": "Aland Islands" }, { "id": "AL", "name": "Albania" }, { "id": "DZ", "name": "Algeria" }, { "id": "AS", "name": "American Samoa" }, { "id": "AD", "name": "Andorra" }, { "id": "AO", "name": "Angola" }, { "id": "AI", "name": "Anguilla" }, { "id": "AG", "name": "Antigua and Barbuda" }, { "id": "AR", "name": "Argentina" }, { "id": "AM", "name": "Armenia" }, { "id": "AW", "name": "Aruba" }, { "id": "AU", "name": "Australia" }, { "id": "AT", "name": "Austria" }, { "id": "AZ", "name": "Azerbaijan" }, { "id": "BS", "name": "Bahamas" }, { "id": "BH", "name": "Bahrain" }, { "id": "BD", "name": "Bangladesh" }, { "id": "BB", "name": "Barbados" }, { "id": "BY", "name": "Belarus" }, { "id": "BE", "name": "Belgium" }, { "id": "BZ", "name": "Belize" }, { "id": "BJ", "name": "Benin" }, { "id": "BM", "name": "Bermuda" }, { "id": "BO", "name": "Bolivia" }, { "id": "BA", "name": "Bosnia and Herzegovina" }, { "id": "BW", "name": "Botswana" }, { "id": "BR", "name": "Brazil" }, { "id": "BN", "name": "Brunei Darussalam" }, { "id": "BG", "name": "Bulgaria" }, { "id": "BF", "name": "Burkina Faso" }, { "id": "Quebec", "name": "CA" }, { "id": "KH", "name": "Cambodia" }, { "id": "CM", "name": "Cameroon" }, { "id": "CA", "name": "Canada" }, { "id": "KY", "name": "Cayman Islands" }, { "id": "Vaud", "name": "CH" }, { "id": "TD", "name": "Chad" }, { "id": "CL", "name": "Chile" }, { "id": "CN", "name": "China" }, { "id": "Anhui Province", "name": "CN" }, { "id": "Jiangxi Province", "name": "CN" }, { "id": "Shaanxi", "name": "CN" }, { "id": "Shaanxi Province", "name": "CN" }, { "id": "CO", "name": "Colombia" }, { "id": "CG", "name": "Congo" }, { "id": "CD", "name": "Congo, The Democratic Republic of the" }, { "id": "CK", "name": "Cook Islands" }, { "id": "CR", "name": "Costa Rica" }, { "id": "CI", "name": "Cote d'Ivoire" }, { "id": "HR", "name": "Croatia" }, { "id": "CU", "name": "Cuba" }, { "id": "CW", "name": "Curacao" }, { "id": "CY", "name": "Cyprus" }, { "id": "CZ", "name": "Czech Republic" }, { "id": "DK", "name": "Denmark" }, { "id": "DM", "name": "Dominica" }, { "id": "DO", "name": "Dominican Republic" }, { "id": "EC", "name": "Ecuador" }, { "id": "EG", "name": "Egypt" }, { "id": "SV", "name": "El Salvador" }, { "id": "ER", "name": "Eritrea" }, { "id": "Catalonia", "name": "ES" }, { "id": "Comunidad Valenciana", "name": "ES" }, { "id": "EE", "name": "Estonia" }, { "id": "ET", "name": "Ethiopia" }, { "id": "FO", "name": "Faroe Islands" }, { "id": "FJ", "name": "Fiji" }, { "id": "FI", "name": "Finland" }, { "id": "Aquitaine", "name": "FR" }, { "id": "Auvergne", "name": "FR" }, { "id": "Bourgogne", "name": "FR" }, { "id": "Bretagne", "name": "FR" }, { "id": "Centre", "name": "FR" }, { "id": "Champagne-Ardenne", "name": "FR" }, { "id": "Haute-Normandie", "name": "FR" }, { "id": "Ile-de-France", "name": "FR" }, { "id": "Languedoc-Roussillon", "name": "FR" }, { "id": "Midi-Pyrenees", "name": "FR" }, { "id": "Nord-Pas-de-Calais", "name": "FR" }, { "id": "Pays de la Loire", "name": "FR" }, { "id": "Picardie", "name": "FR" }, { "id": "Poitou-Charentes", "name": "FR" }, { "id": "Provence-Alpes-Cote", "name": "FR" }, { "id": "Rhone-Alpes", "name": "FR" }, { "id": "FR", "name": "France" }, { "id": "GF", "name": "French Guiana" }, { "id": "PF", "name": "French Polynesia" }, { "id": "GA", "name": "Gabon" }, { "id": "GM", "name": "Gambia" }, { "id": "GE", "name": "Georgia" }, { "id": "DE", "name": "Germany" }, { "id": "GH", "name": "Ghana" }, { "id": "GI", "name": "Gibraltar" }, { "id": "GR", "name": "Greece" }, { "id": "GL", "name": "Greenland" }, { "id": "GP", "name": "Guadeloupe" }, { "id": "GU", "name": "Guam" }, { "id": "GT", "name": "Guatemala" }, { "id": "GG", "name": "Guernsey" }, { "id": "GY", "name": "Guyana" }, { "id": "HT", "name": "Haiti" }, { "id": "HN", "name": "Honduras" }, { "id": "HK", "name": "Hong Kong" }, { "id": "Nograd", "name": "HU" }, { "id": "Pest", "name": "HU" }, { "id": "HU", "name": "Hungary" }, { "id": "IS", "name": "Iceland" }, { "id": "NA", "name": "IL" }, { "id": "IN", "name": "India" }, { "id": "ID", "name": "Indonesia" }, { "id": "IR", "name": "Iran, Islamic Republic of" }, { "id": "IQ", "name": "Iraq" }, { "id": "IE", "name": "Ireland" }, { "id": "IM", "name": "Isle of Man" }, { "id": "IL", "name": "Israel" }, { "id": "IT", "name": "Italy" }, { "id": "JM", "name": "Jamaica" }, { "id": "JP", "name": "Japan" }, { "id": "JE", "name": "Jersey" }, { "id": "JO", "name": "Jordan" }, { "id": "Yamagata Prefecture", "name": "JP" }, { "id": "KZ", "name": "Kazakhstan" }, { "id": "KE", "name": "Kenya" }, { "id": "KP", "name": "Korea, Democratic People's Republic of" }, { "id": "KR", "name": "Korea, Republic of" }, { "id": "KW", "name": "Kuwait" }, { "id": "KG", "name": "Kyrgyzstan" }, { "id": "LA", "name": "Lao People's Democratic Republic" }, { "id": "LV", "name": "Latvia" }, { "id": "LB", "name": "Lebanon" }, { "id": "LR", "name": "Liberia" }, { "id": "LY", "name": "Libyan Arab Jamahiriya" }, { "id": "LI", "name": "Liechtenstein" }, { "id": "LT", "name": "Lithuania" }, { "id": "LU", "name": "Luxembourg" }, { "id": "MO", "name": "Macao" }, { "id": "MK", "name": "Macedonia" }, { "id": "MG", "name": "Madagascar" }, { "id": "MW", "name": "Malawi" }, { "id": "MY", "name": "Malaysia" }, { "id": "MV", "name": "Maldives" }, { "id": "ML", "name": "Mali" }, { "id": "MT", "name": "Malta" }, { "id": "MH", "name": "Marshall Islands" }, { "id": "MQ", "name": "Martinique" }, { "id": "MU", "name": "Mauritius" }, { "id": "YT", "name": "Mayotte" }, { "id": "MX", "name": "Mexico" }, { "id": "MD", "name": "Moldova, Republic of" }, { "id": "MC", "name": "Monaco" }, { "id": "MN", "name": "Mongolia" }, { "id": "ME", "name": "Montenegro" }, { "id": "MA", "name": "Morocco" }, { "id": "MM", "name": "Myanmar" }, { "id": "NA", "name": "Namibia" }, { "id": "NP", "name": "Nepal" }, { "id": "AN", "name": "Netherlands" }, { "id": "NL", "name": "Netherlands" }, { "id": "NC", "name": "New Caledonia" }, { "id": "NZ", "name": "New Zealand" }, { "id": "NI", "name": "Nicaragua" }, { "id": "NE", "name": "Niger" }, { "id": "NG", "name": "Nigeria" }, { "id": "MP", "name": "Northern Mariana Islands" }, { "id": "NO", "name": "Norway" }, { "id": "OM", "name": "Oman" }, { "id": "PK", "name": "Pakistan" }, { "id": "PW", "name": "Palau" }, { "id": "PS", "name": "Palestinian Territory" }, { "id": "PA", "name": "Panama" }, { "id": "PG", "name": "Papua New Guinea" }, { "id": "PY", "name": "Paraguay" }, { "id": "PE", "name": "Peru" }, { "id": "PH", "name": "Philippines" }, { "id": "PN", "name": "Pitcairn" }, { "id": "PL", "name": "Poland" }, { "id": "PT", "name": "Portugal" }, { "id": "PR", "name": "Puerto Rico" }, { "id": "QA", "name": "Qatar" }, { "id": "RE", "name": "Reunion" }, { "id": "RO", "name": "Romania" }, { "id": "RU", "name": "Russian Federation" }, { "id": "KN", "name": "Saint Kitts and Nevis" }, { "id": "LC", "name": "Saint Lucia" }, { "id": "VC", "name": "Saint Vincent and the Grenadines" }, { "id": "WS", "name": "Samoa" }, { "id": "SM", "name": "San Marino" }, { "id": "SA", "name": "Saudi Arabia" }, { "id": "SN", "name": "Senegal" }, { "id": "RS", "name": "Serbia" }, { "id": "SC", "name": "Seychelles" }, { "id": "SG", "name": "Singapore" }, { "id": "SX", "name": "Sint Maarten" }, { "id": "Banska Bystrica Regi", "name": "SK" }, { "id": "SK", "name": "Slovakia" }, { "id": "SI", "name": "Slovenia" }, { "id": "SB", "name": "Solomon Islands" }, { "id": "ZA", "name": "South Africa" }, { "id": "ES", "name": "Spain" }, { "id": "LK", "name": "Sri Lanka" }, { "id": "SD", "name": "Sudan" }, { "id": "SR", "name": "Suriname" }, { "id": "SZ", "name": "Swaziland" }, { "id": "SE", "name": "Sweden" }, { "id": "CH", "name": "Switzerland" }, { "id": "SY", "name": "Syrian Arab Republic" }, { "id": "TW", "name": "Taiwan" }, { "id": "TJ", "name": "Tajikistan" }, { "id": "TZ", "name": "Tanzania, United Republic of" }, { "id": "TH", "name": "Thailand" }, { "id": "TT", "name": "Trinidad and Tobago" }, { "id": "TN", "name": "Tunisia" }, { "id": "TR", "name": "Turkey" }, { "id": "TM", "name": "Turkmenistan" }, { "id": "UG", "name": "Uganda" }, { "id": "UA", "name": "Ukraine" }, { "id": "AE", "name": "United Arab Emirates" }, { "id": "GB", "name": "United Kingdom" }, { "id": "US", "name": "United States" }, { "id": "UY", "name": "Uruguay" }, { "id": "UZ", "name": "Uzbekistan" }, { "id": "VU", "name": "Vanuatu" }, { "id": "VE", "name": "Venezuela" }, { "id": "VN", "name": "Vietnam" }, { "id": "VG", "name": "Virgin Islands, British" }, { "id": "VI", "name": "Virgin Islands, U.S." }, { "id": "YE", "name": "Yemen" }, { "id": "ZM", "name": "Zambia" }, { "id": "ZW", "name": "Zimbabwe" }]
        ...
        QueryBuilder.regional['en'] = {
            ...
            "errors": {
                ...
                "location_empty": "No location specified"
            }
        };
